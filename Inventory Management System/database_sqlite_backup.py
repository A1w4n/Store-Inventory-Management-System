...
class InventoryDatabase:
    ...
    def connect(self):
        ...
        self.connection = sqlite3.connect(str(self.db_path), timeout=15, check_same_thread=False)
    ...
    def record_sale(self, item_id, quantity_sold, user_id=None, sale_price=None):
        ...
        try:
            ...
            # Update quantity directly (avoid nested transaction)
            cursor.execute("""
                UPDATE items SET quantity = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            """, (new_quantity, item_id))
    ...
    def get_items_added_per_day(self, days=7):
        """Get number of items added per day for the last N days."""
        conn = self.connect()
        cursor = conn.cursor()
        cutoff_date = (datetime.now() - timedelta(days=days)).isoformat()
        ...
        # Return as list of tuples: [(date_string, count), ...]
